...
int main(void)
{
    ...
    // Enable clocks
    enablePort(PORTC);
    selectPinPushPullOutput(ENCODER);
    setPinValue(ENCODER, 1);

    //For initialisation of watchdog to see an LED
 ...
 //                   snprintf(str, sizeof(str), "Writing 0x%02"PRIX8" to address 0x%02"PRIX8"\n", data, add);
                    ...
                    {
                        snprintf(str, sizeof(str), "Writing 0x%02"PRIX8" to address 0x%02"PRIX8"\n", data, add);
                        sprintf(LCD_d, "I2C_T UART  0x%02"PRIX8"\n", data);
                        putsLcd(1, 0, LCD_d);
        ...
        if (strcmp(token, "read") == 0)
        {
            ...
            QEI0_MAXPOS_R = 0xFFFFFFFF;   // chosen for int32_t return value
